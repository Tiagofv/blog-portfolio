        ...
        <div class="mb-4 flex justify-between items-center">
            <div class="flex-1 pr-4">
                <div class="relative md:w-1/3" >
                    <input type="search"
                           x-data="{ search: 'false' }" x-init="$watch('search', value => datatables().filter(value))"
                           x-model="search"
...
{{--                    </th>--}}
                ...
                <tbody>
                <template x-for="(d, index) in showData" :key="index">
                        <tr>
                    <template x-for="(heading, index) in headings">
                            <td class="border-dashed border-t border-gray-200 " x-bind:class="{[heading.key]: true}">
                                <span class="text-gray-700 px-6 py-3 flex items-center" x-text="d[heading.key]"></span>
                            </td>
            ...
            return {
                ...
                filter(value){
                    if (value.trim().length > 0) {
                        ...
                        const ids = []
                        this.data.map(obj => {
                            const strRegExPattern = '\\b' + value.toLowerCase() + '.*\\b'
                            Object.values(obj).map(e => {
                                if (e) {
                                    const match = String(e).toLowerCase().match(strRegExPattern)
                                    if (match) {
